"use client";

import { Phone, Mail, MapPin } from "lucide-react";
import { firmDetails, navLinks } from "@/lib/data";
import Image from "next/image";
export default function Footer() {
  const handleScroll = (
    e: React.MouseEvent<HTMLAnchorElement>,
    href: string,
  ) => {
    e.preventDefault();
    const targetId = href.replace("#", "");
    const targetEl = document.getElementById(targetId);
    if (targetEl) {
      const navOffset = 80;
      const elementPosition = targetEl.getBoundingClientRect().top;
      const offsetPosition = elementPosition + window.pageYOffset - navOffset;
      window.scrollTo({ top: offsetPosition, behavior: "smooth" });
    }
  };

  return (
    <footer className="bg-[#24527A] text-slate-300 border-t border-slate-700/80 relative pt-16 pb-8 overflow-hidden">
      {/* Subtle Grid Background */}
      <div className="absolute inset-0 bg-grid-pattern-dark opacity-20 pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-12 pb-14 border-b border-slate-700/80">
          {/* Left Column: Brand & Logo */}
          <div className="lg:col-span-4 space-y-4">
            <a
              href="#hero"
              onClick={(e) => handleScroll(e, "#hero")}
              className="flex items-center gap-3 w-fit"
            >
              <Image
                src="/images/ca-icon.png"
                alt="CA"
                width={32}
                height={32}
                className="object-contain"
              />
              <div className="flex flex-col">
                <span className="text-xl font-bold tracking-tight text-white">
                  Narhari V.Dixit <span className="text-[#3B82C4]">&</span> Co.
                </span>
                <span className="text-[10px] sm:text-[12px] font-medium tracking-widest uppercase text-slate-300">
                  Chartered Accountants
                </span>
              </div>
            </a>

            <p className="text-[12px] sm:text-[14px] text-slate-300 leading-relaxed max-w-sm pt-2 font-normal">
              {firmDetails.tagline} Registered with the Institute of Chartered
              Accountants of India under FRN 108954W. Delivering statutory
              precision, tax optimization, and corporate financial advisory.
            </p>

            <div className="flex items-center gap-3 pt-2">
              <a
                href={firmDetails.socials.linkedin}
                target="_blank"
                rel="noreferrer"
                className="p-2.5 rounded-lg bg-[#24527A] text-slate-200 hover:text-[#3B82C4] transition-colors border border-slate-600/50"
                aria-label="LinkedIn"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M19 3a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14m-.5 15.5v-5.3a3.26 3.26 0 0 0-3.26-3.26c-.85 0-1.84.52-2.28 1.3v-1.11h-2.79v8.37h2.79v-4.93c0-.77.62-1.4 1.39-1.4a1.4 1.4 0 0 1 1.4 1.4v4.93h2.75M6.46 10.9v8.37H9.25V10.9H6.46M7.86 6.75a1.46 1.46 0 1 0 0 2.92 1.46 1.46 0 0 0 0-2.92Z" />
                </svg>
              </a>
              <a
                href={firmDetails.socials.twitter}
                target="_blank"
                rel="noreferrer"
                className="p-2.5 rounded-lg bg-[#24527A] text-slate-200 hover:text-[#3B82C4] transition-colors border border-slate-600/50"
                aria-label="Twitter / X"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href={firmDetails.socials.facebook}
                target="_blank"
                rel="noreferrer"
                className="p-2.5 rounded-lg bg-[#24527A] text-slate-200 hover:text-[#3B82C4] transition-colors border border-slate-600/50"
                aria-label="Facebook"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M12 2.04C6.5 2.04 2 6.53 2 12.06C2 17.06 5.66 21.21 10.44 21.96V14.96H7.9V12.06H10.44V9.85C10.44 7.34 11.93 5.96 14.22 5.96C15.31 5.96 16.45 6.15 16.45 6.15V8.62H15.19C13.95 8.62 13.56 9.39 13.56 10.18V12.06H16.34L15.89 14.96H13.56V21.96A10 10 0 0 0 22 12.06C22 6.53 17.5 2.04 12 2.04Z" />
                </svg>
              </a>
              <a
                href={firmDetails.socials.instagram}
                target="_blank"
                rel="noreferrer"
                className="p-2.5 rounded-lg bg-[#24527A] text-slate-200 hover:text-[#3B82C4] transition-colors border border-slate-600/50"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Center Left: Quick Links */}
          <div className="lg:col-span-2">
            <h4 className="text-[10px] sm:text-[12px] font-medium text-white uppercase tracking-widest mb-4">
              Quick Links
            </h4>
            <ul className="space-y-2.5 text-[12px] sm:text-[14px] font-medium">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    onClick={(e) => handleScroll(e, link.href)}
                    className="hover:text-[#5BA7D1] transition-colors"
                  >
                    {link.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Center Right: Core Services */}
          <div className="lg:col-span-3">
            <h4 className="text-[10px] sm:text-[12px] font-medium text-white uppercase tracking-widest mb-4">
              Practice Areas
            </h4>
            <ul className="space-y-2.5 text-[12px] sm:text-[14px] font-medium">
              <li>
                <a
                  href="#services"
                  onClick={(e) => handleScroll(e, "#services")}
                  className="hover:text-[#5BA7D1] transition-colors"
                >
                  Income Tax Planning & Assessment
                </a>
              </li>
              <li>
                <a
                  href="#services"
                  onClick={(e) => handleScroll(e, "#services")}
                  className="hover:text-[#5BA7D1] transition-colors"
                >
                  GST Audit & Returns Compliance
                </a>
              </li>
              <li>
                <a
                  href="#services"
                  onClick={(e) => handleScroll(e, "#services")}
                  className="hover:text-[#5BA7D1] transition-colors"
                >
                  Statutory & Internal Audit
                </a>
              </li>
              <li>
                <a
                  href="#services"
                  onClick={(e) => handleScroll(e, "#services")}
                  className="hover:text-[#5BA7D1] transition-colors"
                >
                  Corporate Accounting & Bookkeeping
                </a>
              </li>
              <li>
                <a
                  href="#services"
                  onClick={(e) => handleScroll(e, "#services")}
                  className="hover:text-[#5BA7D1] transition-colors"
                >
                  Virtual CFO & Business Valuation
                </a>
              </li>
              <li>
                <a
                  href="#services"
                  onClick={(e) => handleScroll(e, "#services")}
                  className="hover:text-[#5BA7D1] transition-colors"
                >
                  Payroll & TDS Statutory Filings
                </a>
              </li>
            </ul>
          </div>

          {/* Right Column: Direct Contact Info */}
          <div className="lg:col-span-3 space-y-3 text-[12px] sm:text-[14px]">
            <h4 className="text-[10px] sm:text-[12px] font-medium text-white uppercase tracking-widest mb-4">
              Contact Practice Desk
            </h4>

            <div className="flex items-start gap-2.5">
              <MapPin className="w-4 h-4 text-[#3B82C4] shrink-0 mt-0.5" />
              <span className="text-slate-200 leading-snug font-normal">
                {firmDetails.address}
              </span>
            </div>

            <div className="flex items-center gap-2.5">
              <Phone className="w-4 h-4 text-[#3B82C4] shrink-0" />
              <span className="text-slate-200 font-normal">
                {firmDetails.phone}
              </span>
            </div>

            <div className="flex items-center gap-2.5">
              <Mail className="w-4 h-4 text-[#3B82C4] shrink-0" />
              <span className="text-slate-200 font-normal">
                {firmDetails.email}
              </span>
            </div>

            <div className="pt-2 text-[11px] text-slate-400 font-mono">
              Working Hours: {firmDetails.officeHours}
            </div>
          </div>
        </div>

        {/* Bottom Copyright Bar */}
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-[12px] text-slate-400 font-normal">
          <p>
            © 2026 {firmDetails.name} Chartered Accountants. All Rights
            Reserved.
          </p>

          <div className="flex items-center gap-6 text-[11px]">
            <a
              href="#contact"
              onClick={(e) => handleScroll(e, "#contact")}
              className="hover:text-[#5BA7D1] transition-colors"
            >
              Privacy Policy
            </a>
            <a
              href="#contact"
              onClick={(e) => handleScroll(e, "#contact")}
              className="hover:text-[#5BA7D1] transition-colors"
            >
              Terms & Conditions
            </a>
            <a
              href="#contact"
              onClick={(e) => handleScroll(e, "#contact")}
              className="hover:text-[#5BA7D1] transition-colors"
            >
              ICAI Regulatory Disclosures
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}
